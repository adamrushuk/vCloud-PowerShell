# Adam Rush
# Updated on: 2016-12-15 10:48

### Function TODOs ###
## vCloud ##

# COMPLETED #
    # Find Edge View in vCloud using Search-Cloud
    # Get Edge XML config via vCloud API
    # Export full Edge XML
    # Export Edge Services XML
    # Prepare EdgeGatewayServiceConfiguration (VPN / Static Routes) Edge XML
    # Import Edge Services XML
    # Monitor Task progress
    # Import full Edge XML
    # Redeploy Edge
    # Prompt user to confirm correct default gateway IP for Edge

    ## vShield Manager ##
    # Get Edge-ID
    # Get Edge XML via VSM
    # Check Edge default gateway IP
    # - $VSMEdgeGatewayIP = $EdgeXML.edge.features.staticRouting.defaultRoute.gatewayAddress


# TODO #
    # Reparticipate Edge
    # - Get full Edge XML 
    # - Cut SubnetParticipation section (that contains correct Gateway IP) into temp variable    
    # - Cut EdgeGatewayServiceConfiguration section into temp variable (for faster reparticipation attempts)
    # -- Test timings to see if this option is worth it, as may take longer removing and re-adding
    # - Update full Edge config: Import-EdgeFullXML
    # - Check Edge default gateway IP 

    # Can we check for attached peer Edge for VPNs, then back that up too?
...
function Get-EdgeView {
    ...
    .EXAMPLE
       $EdgeView = Get-EdgeView -Name "nft001a4i2 -1"
    .NOTES
        Author: Adam Rush
        Created: 2016-12-08
    #>
...
function Export-EdgeXML {
    ...
    $SavePath = "E:\AdamRush\PS\Edges\EdgeXML\$($EdgeName)"
...
function Import-EdgeServicesXML {
    ...
    .EXAMPLE
        ...
        Import-EdgeServicesXML -Name "nft000xxi2-1" -Path "C:\Path\To\EdgeXML\nft000xxi2-1\nft000xxi2-1_VPN_SR.xml"
    .EXAMPLE
        Connect-CIServer vcloud
        Import-EdgeServicesXML -Name "nft001a4i2 -1" -Path "E:\AdamRush\PS\Edges\EdgeXML\nft001a4i2 -1\nft001a4i2 -1_VPN_SR.xml"
    .EXAMPLE
        Import-EdgeServicesXML -Name "nft001a4i2 -1" -XML $EdgeServicesXML  
    .NOTES
        Author: Adam Rush
        Created: 2016-12-08
    #>
    ...
    $TaskResult.Task | Select operation, startTime, endTime, status | fl
}
...
function Import-EdgeFullXML {
    ...
    .EXAMPLE
        Connect-CIServer api.vcd.portal.skyscapecloud.com
        Import-EdgeFullXML -Name "nft000xxi2-1" -Path "C:\Path\To\EdgeXML\nft000xxi2-1\nft000xxi2-1_BACKUP_2016-12-08_16-37.xml"
    .EXAMPLE
        Import-EdgeFullXML -Name "nft001a4i2 -1" -Path "E:\AdamRush\PS\Edges\EdgeXML\nft001a4i2 -1\nft001a4i2 -1_BACKUP_2016-12-08_16-37.xml"
    .EXAMPLE
        Import-EdgeFullXML -Name "nft001a4i2 -1" -XML $EdgeXML    
    .NOTES
        Author: Adam Rush
        Created: 2016-12-08
    #>
...
function Confirm-EdgeDefaultGateway {
    ...
    $GatewayInterface = $EdgeXML.EdgeGateway.Configuration.GatewayInterfaces.GatewayInterface | Where {$_.UseForDefaultRoute -eq "true"}

    # Get Interface IP addresses
    $IPAdresses = $GatewayInterface.SubnetParticipation | Select -ExpandProperty Gateway

    # Build menu and prompt user
    $Menu = @{}
    for ($i=1;$i -le $IPAdresses.count; $i++) {
...
"@
    ...
    $GatewayInterface = $EdgeXML_UnParticipate.EdgeGateway.Configuration.GatewayInterfaces.GatewayInterface | Where {$_.UseForDefaultRoute -eq "true"}
    $SubnetParticipationNode = $GatewayInterface.SubnetParticipation | Where {$_.Gateway -eq $DefaultGatewayIP}
...
function Start-EdgeReparticipate {
    ...
    .EXAMPLE
         Start-EdgeReparticipate -Name "nft001a4i2 -1" -VSMServer "vfm00002i2"	
    .NOTES
        Author: Adam Rush
        Created: 2016-12-09
    #>
    ...
    # Set the namespace
    <#
    $Namespace = New-Object System.Xml.XmlNamespaceManager($EdgeXML.NameTable)
    $Namespace.AddNamespace("ns", $EdgeXML.EdgeGateway.NamespaceURI)
    #>
...
function Connect-vShieldManager {
    ...
    .EXAMPLE
        Connect-vShieldManager -Server vfm00002i2  	
    .NOTES
        Author: Adam Rush
        Created: 2016-12-09
    #>
...
"@
    ...
    if (-not $Global:VSMConnection) {
        ...
        try {
            Write-Verbose "Logging into $Server API..."

            $Uri = "https://$Server/api/2.0/global/heartbeat/"
            ...
            throw "Error connecting to $Server. Check credentials."
        }
...
function Get-VSMEdge {
    ...
    .EXAMPLE
        Connect-vShieldManager -Server vfm00002i2 
        Get-VSMEdge -EdgeName "nft001a4i2 -1" -Server "vfm00002i2"  	
    .NOTES
        Author: Adam Rush
        Created: 2016-12-09
    #>
    ...
    # Check for vShield Manager connection
    if (-not $Global:VSMConnection) {
        throw "Please connect to vShield Manager before using this function, eg. Connect-vShieldManager -Server vfm00002i2"
    }
    ...
    try {
        ...
        # Find single Edge
        $Edge = $EdgesXML.pagedEdgeList.edgePage.edgeSummary | Where {$_.name -match $EdgeName}
...
function Get-VSMEdgeXML {
    ...
    .EXAMPLE
        Connect-vShieldManager -Server vfm00002i2
        $VSMEdge = Get-VSMEdge -EdgeName "nft001a4i2 -1" -Server "vfm00002i2"
...
# Get Edge Default Gateway IP from VSM
$VSMEdgeGatewayIP = $EdgeXML.edge.features.staticRouting.defaultRoute.gatewayAddress

## GET VCLOUD EDGE ##
# This should return 1 Edge
$EdgeView = Get-EdgeView -Name "nft001a4i2 -1"

# Get Edge XML
$EdgeXML = Get-EdgeXML $EdgeView

# Export Edge XML to file
Export-EdgeXML $EdgeXML

# Export Edge XML to file and prepare VPN / SR XML
Export-EdgeXML $EdgeXML -PrepareXML

---

## GET VSHIELD MANAGER EDGE ##
Connect-vShieldManager -Server vfm00002i2
$VSMEdge = Get-VSMEdge -EdgeName "nft001a4i2 -1" -Server "vfm00002i2"
$VSMEdgeXML = Get-VSMEdgeXML $VSMEdge
...
# This should return NULL as not full name used
Get-EdgeView -Name "nft00036i2"
...
# Export Edge XML
Export-EdgeXML -Name "nft001a4i2 -1" $EdgeXML

---

OLD METHOD
# Get Bryce's test Edge
#Get-EdgeConfig -Name "nft001a4i2 -1" -PrepareXML
